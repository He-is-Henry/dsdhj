...
type Props = {
  formData: InitialManuscript,
  setFormData: Dispatch<SetStateAction<InitialManuscript>>,
  onNext: () => void,
  prevStage: () => void
}

const Stage4 = ({ formData, setFormData, onNext, prevStage }: Props) => {
  const { user } = useAuth();
  ...
  const fileUpdateForbidden = () => {
    ...
    return !(isPaid && isEditor) && !(isScreening && isAuthor);
  };
  const handleFileSelect = (e: ChangeEvent<HTMLInputElement, HTMLInputElement>) => {
  ...
  const prevFilename: string | null = formData.file
    ? decodeURIComponent(formData.file.split("/").pop()?.split('_')[0] || "") || null
    : null;
  ...
  const handleUpload = async () => {
    ...
    if (formData.file) {
      try {
        ...
        setFormData((prev: InitialManuscript) => ({ ...prev, file: "" }));
