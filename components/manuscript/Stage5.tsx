...
const Stage5 = ({
  ...
  const fileURL = formData?.file ? `https://docs.google.com/viewer?url=${encodeURIComponent(
    formData.file,
  )}&embedded=true` : null
  ...
  const defaultOnConfirm = () => {
    setShow(false);
    router.push(isEdit ? `/view/${id}` : "/dashboard");
  };
  ...
  return (
    <section style={styles.section}>
      <ConfirmModal
        open={show}
        message={formData?.title ? `Successfully submitted manuscript "${formData.title.length > 20
